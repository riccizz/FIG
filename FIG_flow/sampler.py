...
class FIG(ReconAlgo):
  def update(self, x, x_next, y, v, i, t, next_t, N, noise_y, **kwargs):
    ...
    # mix_coef = 0.95
    # yt = next_t * y + self.config.w * (1-t) * torch.randn_like(y)  # rescaled version of stochastic interpolant 
    yt = next_t * y + self.config.w * (1-t) * self.operator.H(torch.randn_like(x_next))  # rescaled version of stochastic interpolant 
    # yt = next_t * y + self.config.w * (1-t) * noise_y  # rescaled version of stochastic interpolant 
    if i > 1 and i != N-1:
      for _ in range(self.config.ngd):
