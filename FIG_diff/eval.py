...
@register_eval_fn('fid')
class FrechetInceptionDistance(EvalFn):
    cmp = 'min'  # the higher, the better
    ...
    def evaluate_in_batch(self, gt, pred):
        dl_gt = [{'images':self.norm(gt)}]
        dl_pred = [{'images':self.norm(pred)}]
        first_feats = self.fid_fn.compute_feats(dl_gt)
        second_feats = self.fid_fn.compute_feats(dl_pred)
        results = self.fid_fn(first_feats, second_feats)
        return torch.full((gt.shape[0],), results)

    def __call__(self, gt, measurement, sample, reduction='none'):
        if gt.shape[0] <= 1:
            raise ValueError("Cannot compute FID for less than 2 images. ")
        res = self.evaluate_in_batch(gt, sample)
        if reduction == 'mean':
            res = res.mean()
        return res